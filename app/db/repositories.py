from __future__ import annotationsimport jsonfrom datetime import datetimefrom typing import Anyfrom app.db.connection import db_cursor, json_dumpfrom app.shared.types import IncomingMessage, ParseResult, SourceGroupConfigclass SourceGroupRepository:    def upsert(self, config: SourceGroupConfig, parser_key: str) -> None:        with db_cursor() as cursor:            for chat_id in config.source_chat_ids:                source_thread_ids = config.source_thread_ids(chat_id) or {0}                for source_thread_id in source_thread_ids:                    cursor.execute(                        """                        INSERT INTO source_groups                            (name, source_chat_id, source_thread_id, target_chat_id, target_thread_id, parser_key, filters_json, enabled)                        VALUES (%s, %s, %s, %s, %s, %s, CAST(%s AS JSON), %s)                        ON DUPLICATE KEY UPDATE                            target_chat_id = VALUES(target_chat_id),                            target_thread_id = VALUES(target_thread_id),                            parser_key = VALUES(parser_key),                            filters_json = VALUES(filters_json),                            enabled = VALUES(enabled)                        """,                        (                            config.name,                            chat_id,                            source_thread_id,                            config.target_chat_id,                            config.target_thread_id,                            parser_key,                            json_dump(config.filters.__dict__),                            config.enabled,                        ),                    )class MessageRepository:    def save_incoming(self, message: IncomingMessage) -> tuple[int, bool]:        with db_cursor(dictionary=True) as cursor:            cursor.execute(                """                INSERT INTO incoming_messages                    (group_name, telegram_chat_id, telegram_thread_id, telegram_message_id,                     telegram_reply_to_message_id, message_date, raw_text, raw_json)                VALUES (%s, %s, %s, %s, %s, %s, %s, CAST(%s AS JSON))                ON DUPLICATE KEY UPDATE                    id = LAST_INSERT_ID(id),                    telegram_thread_id = VALUES(telegram_thread_id),                    telegram_reply_to_message_id = VALUES(telegram_reply_to_message_id),                    raw_text = VALUES(raw_text),                    raw_json = VALUES(raw_json)                """,                (                    message.group_name,                    message.chat_id,                    message.thread_id,                    message.message_id,                    message.reply_to_message_id,                    _dt(message.message_date),                    message.text,                    json_dump(message.raw_json),                ),            )            incoming_id = int(cursor.lastrowid)            created = cursor.rowcount == 1            return incoming_id, created    def save_parsed(self, incoming_id: int, parser_key: str, result: ParseResult) -> int:        with db_cursor(dictionary=True) as cursor:            cursor.execute(                """                INSERT INTO parsed_messages                    (incoming_message_id, parser_key, kind, status, reason, payload_json)                VALUES (%s, %s, %s, %s, %s, CAST(%s AS JSON))                ON DUPLICATE KEY UPDATE                    id = LAST_INSERT_ID(id),
                    kind = VALUES(kind),
