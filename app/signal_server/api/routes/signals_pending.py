...
from app.signal_server.auth import check_http_request, require_http_session

router = APIRouter(prefix="/api/signals", tags=["signals"])


@router.get("/pending")
async def pending_signals(request: Request, after_id: int = 0, limit: int = 50, include_rejected: bool = True) -> dict:
