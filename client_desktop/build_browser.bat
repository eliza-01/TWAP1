...
echo.
echo === TWAPs Browser Client build ===
...
"%VENV_PY%" -m pip install -r requirements.txt -r client_desktop\requirements-browser.txt
if errorlevel 1 goto error

echo.
echo Building Browser client...
"%VENV_PY%" -m PyInstaller --noconfirm --clean client_desktop\browser_client.spec
if errorlevel 1 goto error

echo.
echo Done: dist\TWAPs Browser Client.exe
echo.
